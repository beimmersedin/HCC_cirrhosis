...
# 출력 경로
out_dir <- "Deg_80/results_output"

# 1. 데이터 불러오기
# MTX가 cells x genes 형식(Python/AnnData 기준)이므로 transpose 필요
cat("Loading data...\n")
counts_raw <- readMM("data/counts_sparse_80.mtx")
cell_meta <- read.csv("data/cell_metadata.csv", row.names = 1)
gene_meta <- read.csv("data/gene_metadata_80.csv")
...
# 2. Cirrhosis + Endothelial_Cells만 추출 (predicted_celltype 사용)
cat("Subsetting Cirrhosis Endothelial_Cells...\n")
seurat_sub <- subset(seurat_obj, subset = status == "Cirrhosis" & predicted_celltype == "Endothelial_Cells")
rm(seurat_obj)

# nFeature_RNA NA 제거
seurat_sub <- subset(seurat_sub, cells = colnames(seurat_sub)[!is.na(seurat_sub$nFeature_RNA)])
cat(paste0("Cirrhosis Endothelial_Cells after NA removal: ", ncol(seurat_sub), "\n"))
...
# 4. MAST 실행 (NT vs TC)
cat("Running MAST DEG analysis (NT vs TC in Cirrhosis Endothelial_Cells)...\n")
...
write.csv(out_df, file.path(out_dir, "MAST_DEGs_Cirrhosis_NTvsTC_Endothelial.csv"), row.names = FALSE)
cat(paste0("Total DEGs found: ", nrow(out_df), "\n"))
cat(paste0("Significant DEGs (padj < 0.05): ",
           sum(out_df$padj < 0.05), "\n"))

cat("Done! Results saved to Deg_80/results_output/\n")
