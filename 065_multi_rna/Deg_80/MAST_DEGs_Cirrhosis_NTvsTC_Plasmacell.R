...
# 2. Cirrhosis + Plasma_Cells만 추출 (predicted_celltype 사용)
cat("Subsetting Cirrhosis Plasma_Cells...\n")
seurat_sub <- subset(seurat_obj, subset = status == "Cirrhosis" & predicted_celltype == "Plasma_Cells")
rm(seurat_obj)

# nFeature_RNA NA 제거
seurat_sub <- subset(seurat_sub, cells = colnames(seurat_sub)[!is.na(seurat_sub$nFeature_RNA)])
cat(paste0("Cirrhosis Plasma_Cells after NA removal: ", ncol(seurat_sub), "\n"))
...
# 4. MAST 실행 (NT vs TC)
cat("Running MAST DEG analysis (NT vs TC in Cirrhosis Plasma_Cells)...\n")
...
write.csv(out_df, file.path(out_dir, "MAST_DEGs_Cirrhosis_NTvsTC_Plasmacell.csv"), row.names = FALSE)
