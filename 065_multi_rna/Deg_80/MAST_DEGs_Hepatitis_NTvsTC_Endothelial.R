...
# 2. Hepatitis + Endothelial_Cells만 추출 (predicted_celltype 사용)
cat("Subsetting Hepatitis Endothelial_Cells...\n")
seurat_sub <- subset(seurat_obj, subset = status == "Hepatitis" & predicted_celltype == "Endothelial_Cells")
rm(seurat_obj)

# nFeature_RNA NA 제거
seurat_sub <- subset(seurat_sub, cells = colnames(seurat_sub)[!is.na(seurat_sub$nFeature_RNA)])
cat(paste0("Hepatitis Endothelial_Cells after NA removal: ", ncol(seurat_sub), "\n"))
...
# 4. MAST 실행 (NT vs TC)
cat("Running MAST DEG analysis (NT vs TC in Hepatitis Endothelial_Cells)...\n")
...
write.csv(out_df, file.path(out_dir, "MAST_DEGs_Hepatitis_NTvsTC_Endothelial.csv"), row.names = FALSE)
