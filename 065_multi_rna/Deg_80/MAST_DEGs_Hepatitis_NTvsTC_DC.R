...
# 2. Hepatitis + DCs만 추출 (predicted_celltype 사용)
cat("Subsetting Hepatitis DCs...\n")
seurat_sub <- subset(seurat_obj, subset = status == "Hepatitis" & predicted_celltype == "DCs")
rm(seurat_obj)

# nFeature_RNA NA 제거
seurat_sub <- subset(seurat_sub, cells = colnames(seurat_sub)[!is.na(seurat_sub$nFeature_RNA)])
cat(paste0("Hepatitis DCs after NA removal: ", ncol(seurat_sub), "\n"))
...
# 4. MAST 실행 (NT vs TC)
cat("Running MAST DEG analysis (NT vs TC in Hepatitis DCs)...\n")
...
write.csv(out_df, file.path(out_dir, "MAST_DEGs_Hepatitis_NTvsTC_DC.csv"), row.names = FALSE)
