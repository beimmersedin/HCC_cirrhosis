# =============================================================================
#  TF Motif Analysis Pipeline — TC Hepatocytes (Cirrhosis vs Hepatitis)
#
#  Step 1: 후보 TF 선정 (Motif Enrichment)       → Hepatocyte_Motif_Enrichment.csv
#  Step 2: 결합 부위 확정 (Peak-TF Mapping)       → Hepatocyte_Peak_TF_Match_Map.csv
#  Step 3: 세포별 활성 검증 (chromVAR)             → Hepatocyte_TF_activity.csv
# =============================================================================
...
write.csv(enriched_motifs, paste0(output_dir, "Hepatocyte_Motif_Enrichment.csv"))
message("저장 완료: Hepatocyte_Motif_Enrichment.csv (", nrow(enriched_motifs), " motifs)")
...
# # Step 1에서 유의한 motif만 선별하여 필터링 (주석 처리: 692개 motif 전체 유지)
# sig_motifs <- enriched_motifs$motif[enriched_motifs$p.adjust < 0.05]  # 291
# message("Enriched motifs (p.adjust < 0.05): ", length(sig_motifs), " / ", nrow(enriched_motifs))
# peak_tf_mapping <- peak_tf_mapping[peak_tf_mapping$motif_id %in% sig_motifs, ]

write.csv(peak_tf_mapping, paste0(output_dir, "Hepatocyte_Peak_TF_Match_Map.csv"), row.names = FALSE)
message("저장 완료: Hepatocyte_Peak_TF_Match_Map.csv (", nrow(peak_tf_mapping), " pairs)")
...
write.csv(as.matrix(tf_scores), paste0(output_dir, "Hepatocyte_TF_activity.csv"))
message("저장 완료: Hepatocyte_TF_activity.csv")
...
message(strrep("=", 70))
message("\n출력 파일:")
message("  [Step 1] Hepatocyte_Motif_Enrichment.csv")
message("  [Step 2] Hepatocyte_Peak_TF_Match_Map.csv")
message("  [Step 3] Hepatocyte_TF_activity.csv")
