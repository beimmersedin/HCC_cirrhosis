# =============================================================================
#  TF Motif Analysis Pipeline — TC DCs (Cirrhosis vs Hepatitis)
#
#  Step 1: 후보 TF 선정 (Motif Enrichment)       → DCs_Motif_Enrichment.csv
#  Step 2: 결합 부위 확정 (Peak-TF Mapping)       → DCs_Peak_TF_Match_Map.csv
#  Step 3: 세포별 활성 검증 (chromVAR)             → DCs_TF_activity.csv
# =============================================================================
...
tc_subset     <- subset(multiome_atac, subset = group == "TC" & celltype_assign == "DCs")
...
# DA 결과 로드
sig_dars <- read.csv(paste0(da_dir, "Significant_DARs_DCs.csv"), row.names = 1)
...
write.csv(enriched_motifs, paste0(output_dir, "DCs_Motif_Enrichment.csv"))
message("저장 완료: DCs_Motif_Enrichment.csv (", nrow(enriched_motifs), " motifs)")
...
dar_motifs   <- motif_matrix[common_peaks, , drop = FALSE]
...
write.csv(peak_tf_mapping, paste0(output_dir, "DCs_Peak_TF_Match_Map.csv"), row.names = FALSE)
message("저장 완료: DCs_Peak_TF_Match_Map.csv (", nrow(peak_tf_mapping), " pairs)")
...
write.csv(as.matrix(tf_scores), paste0(output_dir, "DCs_TF_activity.csv"))
message("저장 완료: DCs_TF_activity.csv")
...
message(strrep("=", 70))
message("\n출력 파일:")
message("  [Step 1] DCs_Motif_Enrichment.csv")
message("  [Step 2] DCs_Peak_TF_Match_Map.csv")
message("  [Step 3] DCs_TF_activity.csv")
