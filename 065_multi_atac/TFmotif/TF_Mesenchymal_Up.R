# =============================================================================
#  TF Motif Analysis Pipeline — TC Mesenchymal (Cirrhosis vs Hepatitis)
#
#  Step 1: 후보 TF 선정 (Motif Enrichment)       → Mesenchymal_Motif_Enrichment.csv
#  Step 2: 결합 부위 확정 (Peak-TF Mapping)       → Mesenchymal_Peak_TF_Match_Map.csv
#  Step 3: 세포별 활성 검증 (chromVAR)             → Mesenchymal_TF_activity.csv
# =============================================================================
...
tc_subset     <- subset(multiome_atac, subset = group == "TC" & celltype_assign == "Mesenchymal")
...
# DA 결과 로드
sig_dars <- read.csv(paste0(da_dir, "Significant_DARs_Mesenchymal.csv"), row.names = 1)
...
write.csv(enriched_motifs, paste0(output_dir, "Mesenchymal_Motif_Enrichment.csv"))
message("저장 완료: Mesenchymal_Motif_Enrichment.csv (", nrow(enriched_motifs), " motifs)")
...
write.csv(peak_tf_mapping, paste0(output_dir, "Mesenchymal_Peak_TF_Match_Map.csv"), row.names = FALSE)
message("저장 완료: Mesenchymal_Peak_TF_Match_Map.csv (", nrow(peak_tf_mapping), " pairs)")
...
write.csv(as.matrix(tf_scores), paste0(output_dir, "Mesenchymal_TF_activity.csv"))
message("저장 완료: Mesenchymal_TF_activity.csv")
...
message(strrep("=", 70))
message("\n출력 파일:")
message("  [Step 1] Mesenchymal_Motif_Enrichment.csv")
message("  [Step 2] Mesenchymal_Peak_TF_Match_Map.csv")
message("  [Step 3] Mesenchymal_TF_activity.csv")
