# =============================================================================
#  TF Motif Analysis Pipeline — TC Hepatocytes DOWN-DARs (Hepatitis-open)
#
#  Step 1: 후보 TF 선정 (Motif Enrichment)       → Hepatocyte_Down_Motif_Enrichment.csv
#  Step 2: 결합 부위 확정 (Peak-TF Mapping)       → Hepatocyte_Down_Peak_TF_Match_Map.csv
#  Step 3: 세포별 활성 검증 (chromVAR)             → Hepatocyte_Down_TF_activity.csv
#
#  주의: Down-DAR은 2개뿐이므로 Step 1 enrichment 결과는 통계적으로 제한적
# =============================================================================
...
# DA 결과 로드 — Down-DAR만 추출
sig_dars   <- read.csv(paste0(da_dir, "Significant_DARs_Hepatocytes.csv"), row.names = 1)
down_peaks <- rownames(sig_dars[sig_dars$avg_log2FC < 0, ])
message("Down-regulated DARs (Hepatitis-open): ", length(down_peaks), " peaks")

# ─── Significant DARs에 대한 PWM Score 계산 ──────────────────────────────────
message("PWM Score 계산 중 (Down-DARs only)...")
down_dars   <- sig_dars[sig_dars$avg_log2FC < 0, ]
dar_ranges  <- StringToGRanges(rownames(down_dars), sep = c("-", "-"))
...
rownames(pwm_score_matrix) <- rownames(down_dars)
rownames(pwm_match_matrix) <- rownames(down_dars)
...
# =============================================================================
message("\n", strrep("=", 70))
message("Step 1: 후보 TF 선정 (Motif Enrichment) — Down-DARs")
message("주의: Down-DAR ", length(down_peaks), "개로 enrichment 결과는 통계적으로 제한적")
message(strrep("=", 70))

enriched_motifs <- FindMotifs(object = tc_subset, features = down_peaks)
write.csv(enriched_motifs, paste0(output_dir, "Hepatocyte_Down_Motif_Enrichment.csv"))
message("저장 완료: Hepatocyte_Down_Motif_Enrichment.csv (", nrow(enriched_motifs), " motifs)")

n_sig <- sum(enriched_motifs$p.adjust < 0.05)
message("유의한 motif (p.adjust < 0.05): ", n_sig, " / ", nrow(enriched_motifs))


# =============================================================================
# Step 2. 결합 부위 확정 (Peak-TF Mapping) — Down-DARs only
# =============================================================================
message("\n", strrep("=", 70))
message("Step 2: 결합 부위 확정 (Peak-TF Mapping) — Down-DARs")
message(strrep("=", 70))
...
# Down-DARs만 필터링
common_peaks <- intersect(rownames(down_dars), rownames(motif_matrix))
...
peak_tf_mapping$avg_log2FC <- down_dars[peak_tf_mapping$peak, "avg_log2FC"]
peak_tf_mapping$p_val_adj  <- down_dars[peak_tf_mapping$peak, "p_val_adj"]
...
write.csv(peak_tf_mapping, paste0(output_dir, "Hepatocyte_Down_Peak_TF_Match_Map.csv"), row.names = FALSE)
message("저장 완료: Hepatocyte_Down_Peak_TF_Match_Map.csv (", nrow(peak_tf_mapping), " pairs)")
...
write.csv(as.matrix(tf_scores), paste0(output_dir, "Hepatocyte_Down_TF_activity.csv"))
message("저장 완료: Hepatocyte_Down_TF_activity.csv")
...
# =============================================================================
message("\n", strrep("=", 70))
message("TF 분석 (Down-DAR) 완료! 결과 위치: ", output_dir)
message(strrep("=", 70))
message("\n출력 파일:")
message("  [Step 1] Hepatocyte_Down_Motif_Enrichment.csv")
message("  [Step 2] Hepatocyte_Down_Peak_TF_Match_Map.csv")
message("  [Step 3] Hepatocyte_Down_TF_activity.csv")
message("\n주의: Down-DAR은 ", length(down_peaks), "개뿐이므로 Step 1 결과 해석에 주의 필요")
