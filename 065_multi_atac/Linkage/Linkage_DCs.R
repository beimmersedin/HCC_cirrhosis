# =============================================================================
#  TF-Target Gene 매칭 — TC DCs
#  기존 Peak-Gene Links (LinkPeaks 결과)와 TFmotif Peak-TF Mapping을 조인
#  ※ LinkPeaks는 LinkPeaks_DCs.R에서 별도 수행 (이미 완료된 경우 재실행 불필요)
# =============================================================================
...
# ─── 데이터 로드 ─────────────────────────────────────────────────────────────
message("=== TF-Target Gene 매칭 시작 (DCs) ===")

# Peak-Gene Links (LinkPeaks 결과, 이미 저장된 CSV)
all_links <- fread(paste0(linkage_dir, "DCs_Peak_Gene_Links.csv"), data.table = FALSE)
message("Peak-Gene Links 로드 완료: ", nrow(all_links), " links")

# Peak-TF Mapping (TFmotif 결과)
peak_tf_mapping <- fread(paste0(tf_dir, "DCs_Peak_TF_Match_Map.csv"), data.table = FALSE)
...
write.csv(tf_gene_links, paste0(output_dir, "DCs_TF_Peak_Gene_Links.csv"), row.names = FALSE)
message("저장 완료: DCs_TF_Peak_Gene_Links.csv (", nrow(tf_gene_links), " TF-Peak-Gene triplets)")
...
message(strrep("=", 70))
message("\n출력 파일:")
message("  DCs_TF_Peak_Gene_Links.csv   — TF → Peak → Gene 통합 매핑")
