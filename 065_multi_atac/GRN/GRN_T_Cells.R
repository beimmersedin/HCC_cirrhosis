# =============================================================================
#  GRN (Gene Regulatory Network) — TC T_Cells
...
triplets <- fread(paste0(linkage_dir, "T_Cells_TF_Peak_Gene_Links.csv"), data.table = FALSE)
message("Triplets 로드 완료: ", nrow(triplets), " rows")

# (B) 세포별 TF Activity Score (chromVAR 결과, motif × cell)
tf_scores <- fread(paste0(tf_dir, "T_Cells_TF_activity.csv"), data.table = FALSE)
...
# -----------------------------------------------------------------------------
# 4. 공통 세포 정렬 (tf_scores와 rna_counts 모두에 존재하는 TC T_Cells)
# -----------------------------------------------------------------------------
tc_cells <- rownames(metadata[metadata$group == "TC" & metadata$celltype_assign == "T_Cells", ])
...
write.csv(all_grn, paste0(output_dir, "T_Cells_All_GRN_Weights.csv"), row.names = FALSE)
...
write.csv(final_grn, paste0(output_dir, "T_Cells_Final_Weighted_GRN.csv"), row.names = FALSE)
...
write.csv(master_regulators, paste0(output_dir, "T_Cells_Master_Regulators_Summary.csv"), row.names = FALSE)
...
message("\n결과 위치: ", output_dir)
message("  T_Cells_All_GRN_Weights.csv              — 전체 TF-Gene 상관 (causal evidence 입력용)")
message("  T_Cells_Final_Weighted_GRN.csv           — TF-Gene edge list (weight > 0.1)")
message("  T_Cells_Master_Regulators_Summary.csv    — TF별 타겟 유전자 수")
